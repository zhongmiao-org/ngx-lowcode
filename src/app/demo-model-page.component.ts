    ...
    <div class="workspace-page">
      <thy-card thyBordered="false" class="workspace-page__panel">
        <div class="workspace-page__header">
          ...
          <div class="workspace-page__actions">
            ...
            <button thyButton="outline-primary" size="sm" (click)="workspace.addForeignKey()">
              {{ copy().addForeignKey }}
            </button>
            <button thyButton="outline-primary" size="sm" (click)="workspace.rebuildModelPreset()">
              {{ copy().createModel }}
            </button>
          </div>
        </div>
        ...
        @for (table of workspace.metaModel().tables; track table.id) {
          <div class="workspace-page__card">
            <div class="workspace-page__header">
              ...
              <div class="workspace-page__actions">
                <button thyButton="outline-primary" size="sm" (click)="workspace.addColumn(table.id)">
                  {{ copy().addColumn }}
                </button>
                <button thyButton="outline-primary" size="sm" (click)="workspace.addChildTable(table.id)">
                  {{ copy().addChildTable }}
                </button>
                <button thyButton="outline-primary" size="sm" (click)="workspace.addIndex(table.id)">
                  {{ copy().addIndex }}
                </button>
            ...
            <div class="workspace-page__list">
              @for (column of table.columns; track column.id) {
                <div class="workspace-page__row">
                  ...
                  <thy-tags>
                    <thy-tag
                      [thyColor]="column.primary ? 'danger' : column.required ? 'success' : 'default'"
                      thyTheme="weak-fill"
                      thySize="sm"
                    >
              ...
              @for (relation of workspace.metaModel().relations; track relation.id) {
                <div class="workspace-page__row">
                  ...
                  <select
                    ...
                    [value]="relation.kind"
                    (change)="
                      workspace.updateRelation(relation.id, { kind: toRelationKind($any($event.target).value) })
                    "
                  >
                  ...
                  <button thyButton="outline-danger" size="sm" (click)="workspace.removeRelation(relation.id)">
                    Delete
                  </button>
                </div>
              }
      ...
      <thy-card thyBordered="false" class="workspace-page__panel">
        ...
        @if (workspace.validationIssues().length === 0) {
          <thy-result
            thyStatus="success"
            [thyTitle]="copy().noValidationIssues"
            [thySubtitle]="copy().routeConfigDescription"
          ></thy-result>
        } @else {
          <div class="workspace-page__list">
